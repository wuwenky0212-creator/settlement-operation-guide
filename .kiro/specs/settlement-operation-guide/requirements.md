...
- 下一步操作：等待结算员完成复核
- 操作入口：无需操作
- 注意事项：系统将在结算员完成复核后自动更新状态
- 预计时间：根据结算员工作安排而定

**场景2: 后台复核 - 交易已删除**
- 当前状态：交易已被结算员删除，流程已终结
- 下一步操作：流程已终结，交易已进入归档库
- 操作入口：无需操作
- 注意事项：已删除的交易无法恢复
- 预计时间：无
...
- 下一步操作：等待RMC回执
- 操作入口：无需操作
- 注意事项：系统正在处理SWIFT链路回执，请稍候
- 预计时间：1-3分钟

**场景4: SWIFT证实 - RMC发送失败**
- 当前状态：RMC回执失败，报文出口受阻，流程已阻断
- 下一步操作：检查报文规范，联系SWIFT技术支持
- 操作入口：提供"SWIFT系统监控"链接
- 注意事项：RMC失败会阻断整个流程，无FTM回执
- 预计时间：需人工处理
...
- 下一步操作：在待办任务中进行手工匹配或执行撤销处理
- 操作入口：提供"人工匹配处理"按钮、"查看匹配差异"链接
- 注意事项：请核对交易信息与对手方报文的差异
- 预计时间：需人工处理
...
**场景11: 清算轧差 - 待轧差**
- 当前状态：前置触发条件已满足，等待系统自动轧差或手动执行轧差操作
- 下一步操作：等待系统日终批处理自动轧差，或手动执行轧差操作
- 操作入口：提供"手工轧差"按钮（如需手动处理）
- 注意事项：系统将在日终批处理时自动执行轧差
- 预计时间：日终批处理时间
...
**场景14: 合规准入 - 合规校验中**
- 当前状态：系统正在进行反洗钱（AML）和黑名单扫描
- 下一步操作：等待反洗钱系统审核完成
- 操作入口：提供"反洗钱系统查询"链接
- 注意事项：系统正在进行反洗钱和黑名单扫描
- 预计时间：1-5分钟

**场景15: 合规准入 - 合规拦截**
- 当前状态：反洗钱检查失败，系统已硬性阻断后续流程
- 下一步操作：联系合规部门，提供补充材料或申请人工审核
- 操作入口：提供"申请人工审核"按钮
- 注意事项：反洗钱检查失败，已硬性阻断后续流程
- 预计时间：需人工处理
...
- 当前状态：RMC发送失败，报文出口受阻
- 下一步操作：检查RMC连接状态，联系SWIFT技术支持
- 操作入口：提供"SWIFT系统监控"链接、"重新发送"按钮
- 注意事项：RMC失败会阻断整个流程，需人工介入排查
- 预计时间：需人工处理
...
- 当前状态：FTM发送失败，报文传输中断
- 下一步操作：检查报文格式，在待办中执行"补发处理"
- 操作入口：提供"重新发送FTM"按钮、"查看报文详情"链接
- 注意事项：报文传输中断（FTM失败），需要补发处理
- 预计时间：需人工处理
...
- 操作入口：提供"内部账划转查询"按钮、"核心系统工单"链接
- 注意事项：核心入账失败，需要人工介入处理
- 预计时间：需人工处理

**场景28: 结算执行 - 入账不明**
- 当前状态：核心入账状态不明，需要人工确认
- 下一步操作：查询内部账划转情况，确认入账状态
- 操作入口：提供"内部账划转查询"按钮
- 注意事项：核心入账状态不明，需要人工确认
- 预计时间：需人工处理
...
- 当前状态：撤销RMC发送失败
- 下一步操作：检查连接状态，联系SWIFT技术支持
- 操作入口：提供"SWIFT系统监控"链接、"重新发送"按钮
- 注意事项：撤销RMC发送失败，需人工介入
- 预计时间：需人工处理
